...
bdRevs[, c("name", "alt_name")]<- lapply(bdRevs[, c("name", "alt_name")], function(x){
  x<- gsub("^(avenida|calle|camino|parque|plaza) ", "camino ", x)
  x<- gsub("^(Avenida|Calle|Camino|Parque|Plaza) ", "Camino ", x)
  x
})
bdRevs[, c("name:ca", "alt_name:ca")]<- lapply(bdRevs[, c("name:ca", "alt_name:ca")], function(x){
  x<- gsub("^(avinguda|carrer|camí|parc|plaça) ", "camí ", x)
  x<- gsub("^(Avinguda|Carrer|Camí|Parc|Plaça) ", "Camí ", x)
  x
})
...
cmd<- paste0(cmd, " --changeset-hashtags \"#toponimsCat;#camino_camí\"",
            " --batch 100 --changeset-comment \"Afegeix name:ca a camins de ", nomComarca, "\"")
cat(cmd, sep="\n")
